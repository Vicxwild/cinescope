...
class TestPost:
    ...
    def test_negative_with_empty_data(self, api_manager):
        post_resp = api_manager.movies_api.create_movie({}, expected_status=400)
        post_data = post_resp.json()

        assert post_resp.status_code == 400
        assert "message" in post_data

    def test_negative_with_empty_name(self, authenticated_admin, api_manager, movie_test_data):
        movie_test_data.update({"name": ""})
        post_resp = api_manager.movies_api.create_movie(movie_test_data, expected_status=400)
        post_data = post_resp.json()
